# Установка и подключение ClickHouse + DBeaver на ALT Linux

![ALT Linux](https://img.shields.io/badge/ALT_Linux-Workstation_K_11.2-0072C6?style=for-the-badge&logo=linux&logoColor=white)
![ClickHouse](https://img.shields.io/badge/ClickHouse-25.8.10.7-FFCC00?style=for-the-badge&logo=clickhouse&logoColor=black)
![DBeaver](https://img.shields.io/badge/DBeaver-25.3.4-4A90D9?style=for-the-badge&logo=dbeaver&logoColor=white)
![Status](https://img.shields.io/badge/Статус-Готово-brightgreen?style=for-the-badge)

---

## 📋 Содержание

- [Требования](#-требования)
- [Установка DBeaver Community](#-установка-dbeaver-community)
- [Установка ClickHouse](#-установка-clickhouse)
- [Настройка пароля для пользователя default](#-настройка-пароля-для-пользователя-default)
- [Подключение DBeaver к ClickHouse](#-подключение-dbeaver-к-clickhouse)
- [Проверка](#-проверка)
- [Смена пароля пользователя default](#-смена-пароля-пользователя-default)

---

## 🖥 Требования

| Компонент | Версия |
|-----------|--------|
| ОС | ALT Workstation K 11.2 (Nemorosa) x86_64 |
| Пакетный менеджер | apt-get (ALT Linux) |
| Flatpak | для установки DBeaver |
| ClickHouse | 25.8.10.7-alt1 (из репозитория p11) |
| DBeaver Community | 25.3.4+ |

---

## 📦 Установка DBeaver Community

![Flatpak](https://img.shields.io/badge/Установка_через-Flatpak-4A90D9?style=flat-square&logo=flatpak)

DBeaver Community Edition отсутствует в официальных репозиториях ALT Linux, поэтому устанавливаем через **Flatpak** из **Flathub**:

```bash
flatpak install flathub io.dbeaver.DBeaverCommunity
```

> 💡 Убедитесь, что Flatpak установлен и репозиторий Flathub подключён.

---

## 🗄 Установка ClickHouse

![ClickHouse](https://img.shields.io/badge/Репозиторий-ALT_Linux_p11-0072C6?style=flat-square)

ClickHouse доступен в **официальных репозиториях ALT Linux p11** — сторонние репозитории не нужны.

### Шаг 1 — Обновить список пакетов

```bash
sudo apt-get update
```

### Шаг 2 — Установить ClickHouse

```bash
sudo apt-get install clickhouse-server clickhouse-client
```

Будут установлены три пакета:
- `clickhouse-common-static` — общие файлы (~82 МБ)
- `clickhouse-client` — клиент командной строки
- `clickhouse-server` — сервер (~67 МБ)

> ⚠️ Суммарный объём после установки: ~834 МБ

### Шаг 3 — Запустить и добавить в автозагрузку

```bash
sudo systemctl enable --now clickhouse-server
```

### Шаг 4 — Проверить статус

```bash
sudo systemctl status clickhouse-server
```

### Шаг 5 — Подключиться через CLI-клиент

```bash
clickhouse-client
```

Успешное подключение выглядит так:

```
ClickHouse client version 25.8.10.1.
Connecting to localhost:9000 as user default.
Connected to ClickHouse server version 25.8.10.

Aquarius :)
```

Для выхода из клиента:

```sql
exit
```

---

## 🔐 Настройка пароля для пользователя `default`

![Security](https://img.shields.io/badge/Безопасность-Обязательно-red?style=flat-square)

В версии ClickHouse 25.x пароль для подключения **обязателен**. Пользователь `default` хранится в read-only конфиге, поэтому пароль задаётся через файл в директории `users.d`.

### Шаг 1 — Создать директорию (если не существует)

```bash
sudo mkdir -p /etc/clickhouse-server/users.d
```

### Шаг 2 — Создать файл с паролем

```bash
sudo nano /etc/clickhouse-server/users.d/default-password.xml
```

Вставить содержимое:

```xml
<clickhouse>
    <users>
        <default>
            <password>password123</password>
        </default>
    </users>
</clickhouse>
```

Сохранить: `Ctrl+O` → `Enter` → `Ctrl+X`

### Шаг 3 — Перезапустить сервер

```bash
sudo systemctl restart clickhouse-server
```

### Шаг 4 — Проверить подключение с паролем

```bash
clickhouse-client --user default --password password123
```

---

## 🔌 Подключение DBeaver к ClickHouse

### Шаг 1 — Создать новое соединение

Открыть DBeaver и нажать **База данных → Новое соединение с базой данных** или `Shift+Ctrl+N`.

### Шаг 2 — Выбрать драйвер ClickHouse

В списке драйверов найти и выбрать **ClickHouse** (не ClickHouse Legacy), нажать **Далее**.

### Шаг 3 — Скачать JDBC-драйвер

DBeaver предложит скачать официальный JDBC-драйвер ClickHouse. Нажать **Скачать**.

> Будет загружен: `ClickHouse JDBC Driver 0.9.5`

### Шаг 4 — Заполнить параметры подключения

| Параметр | Значение |
|----------|----------|
| Хост | `localhost` |
| Порт | `8123` |
| БД/Схема | *(оставить пустым)* |
| Пользователь | `default` |
| Пароль | `password123` |

### Шаг 5 — Протестировать соединение

Нажать **Тест соединения ...**.

Успешный результат:

```
✅ Соединено (188 мс)
Сервер:   ClickHouse 25.8.10.1
Драйвер:  ClickHouse JDBC Driver 0.9.5
```

### Шаг 6 — Сохранить соединение

Нажать **Готово**. Соединение `localhost:8123` появится в панели **Базы данных**.

---

## ✅ Проверка

После успешного подключения в панели **Базы данных** отобразится соединение с иконкой ClickHouse. Можно раскрыть его и увидеть системные базы данных:

- `default`
- `information_schema`
- `system`

---

## 🗒 Полезные команды

```bash
# Статус сервера
sudo systemctl status clickhouse-server

# Перезапуск
sudo systemctl restart clickhouse-server

# Подключение через CLI
clickhouse-client --user default --password password123

# Версия
clickhouse-client --version
```

---

## 📌 Примечания

> ![Note](https://img.shields.io/badge/⚠️_Важно-Безопасность-orange?style=flat-square)
> 
> Пароль `password123` используется в качестве примера. В рабочей среде используйте надёжный пароль.

> ![Note](https://img.shields.io/badge/ℹ️_Порты-Информация-blue?style=flat-square)
> 
> ClickHouse использует два порта: **8123** (HTTP, для JDBC/DBeaver) и **9000** (нативный протокол, для CLI-клиента).

---

*Инструкция подготовлена для ALT Workstation K 11.2 (Nemorosa) x86_64*

## 🔄 Смена пароля пользователя `default`

![Security](https://img.shields.io/badge/Рекомендуется-SHA256-green?style=flat-square)

### Вариант 1 — Через файл конфига (plaintext)

> ⚠️ Простой способ, но пароль хранится в открытом виде. Подходит для локальной разработки.

**Шаг 1** — Отредактировать файл:

```bash
sudo nano /etc/clickhouse-server/users.d/default-password.xml
```

Заменить содержимое на новый пароль:

```xml
<clickhouse>
    <users>
        <default>
            <no_password remove="1"/>
            <password>новый_пароль</password>
        </default>
    </users>
</clickhouse>
```

**Шаг 2** — Перезапустить сервер:

```bash
sudo systemctl restart clickhouse-server
```

**Шаг 3** — Обновить пароль в DBeaver: правой кнопкой на `localhost` → **Редактировать соединение** → ввести новый пароль → **Готово**.

---

### Вариант 2 — SHA256-хэш (рекомендуется) ✅

> 💡 Безопасный способ — пароль в открытом виде нигде не хранится.

**Шаг 1** — Сгенерировать хэш нового пароля:

```bash
echo -n "новый_пароль" | sha256sum | awk '{print $1}'
```

Пример вывода:
```
7f065bc66a5e2a87899ca3c8ac6afe12a8968815c6adf0c5d6725576b458c699
```

> ⚠️ Убедитесь что хэш содержит ровно **64 символа** — иначе сервер не запустится.

**Шаг 2** — Записать хэш в файл конфига одной командой:

```bash
echo -n "новый_пароль" | sha256sum | awk '{print $1}' | xargs -I{} sudo sh -c 'cat > /etc/clickhouse-server/users.d/default-password.xml << EOF
<clickhouse>
    <users>
        <default>
            <no_password remove="1"/>
            <password_sha256_hex>{}</password_sha256_hex>
        </default>
    </users>
</clickhouse>
EOF'
```

**Шаг 3** — Проверить файл (хэш должен быть 64 символа):

```bash
sudo cat /etc/clickhouse-server/users.d/default-password.xml
```

**Шаг 4** — Перезапустить сервер:

```bash
sudo systemctl restart clickhouse-server
```

**Шаг 5** — Обновить пароль в DBeaver: правой кнопкой на `localhost` → **Редактировать соединение** → ввести новый пароль → **Готово**.

> ⚠️ **Важно:** Если сервер не запускается после смены пароля — проверьте файл `/etc/clickhouse-server/users.xml`. В нём не должно быть тега `<password></password>`. Если он есть — выполните:
> ```bash
> sudo sed -i 's|<password></password>|<no_password/>|' /etc/clickhouse-server/users.xml
> sudo systemctl restart clickhouse-server
> ```
